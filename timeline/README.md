...
**What it shows.** The title; the author line, with the year as the link to the timeline, followed by the colour mode and framebuffer depth ("Thant Tessman, 1986 · Colormapped, 8 bitplanes"), and on the right of the same line the mid-range and high-end SGI machine families of the year ("IRIS 2400 · IRIS 3130"); a two-sentence blurb; the inputs the demo listens for, drawn as a three-button mouse with the used buttons lit, "drag" when it reads mouse motion, then the keys, comma separated, lower case for named keys (esc, shift, arrow keys) and upper case for letters, then keypad, dial box, or spaceball; and a source link to the demo's directory on GitHub.

**Where it comes from.** `demos/<name>/placard.json` holds the text: `title`, `author`, `year`, `color`, `depth`, `machines`, `blurb`, `source`, `more` (the timeline URL), and `inputs_detected`. A hand-written `inputs` list overrides the detected one. Authors are named alone; "Unattributed" where the source records none; a non-SGI origin is stated (David A. Tristram, NASA Ames).
