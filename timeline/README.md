...
**Where it comes from.** `demos/<name>/placard.json` holds the text: `title`, `author`, `year`, `color`, `depth`, `buffering`, `hidden`, `api`, `machines`, `blurb`, `source`, `more` (the timeline URL), and `inputs_detected`. A hand-written `inputs` list overrides the detected one. Authors are named alone; "Unattributed" where the source records none; a non-SGI origin is stated (David A. Tristram, NASA Ames).
